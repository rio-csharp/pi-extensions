...
import { subagentTimelineRenderer } from "./timeline-renderer.ts";
	...
	try {
		...
		const exitCode = await new Promise<number>((resolve) => {
			...
			const proc = spawn(invocation.command, invocation.args, {
				...
				env: { ...process.env, PI_SUBAGENT_ACTIVE: "1" },
			});
			let buffer = "";
...
export default function (pi: ExtensionAPI) {
	// Recursion guard: when a subagent process runs, do not expose the subagent
	// tool again, so children cannot spawn their own subagents.
	if (process.env.PI_SUBAGENT_ACTIVE === "1") return;
	...
	pi.registerTool({
		...
		description: [
			...
			"Choose a concise, descriptive title for every subagent so its live status is identifiable.",
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			...
			if (params.chain && params.chain.length > 0) {
				...
				for (let i = 0; i < params.chain.length; i++) {
					...
					const result = await whileRunning(step.agent, step.title, ctx, (onStatusUpdate) =>
						runSingleAgent(
							ctx.cwd,
							agents,
							step.agent,
			...
			if (params.tasks && params.tasks.length > 0) {
				...
				const results = await mapWithConcurrencyLimit(params.tasks, MAX_CONCURRENCY, async (t, index) => {
					const result = await whileRunning(t.agent, t.title, ctx, (onStatusUpdate) =>
						runSingleAgent(
							ctx.cwd,
							agents,
							t.agent,
			...
			if (params.agent && params.task) {
				const result = await whileRunning(params.agent, params.title, ctx, (onStatusUpdate) =>
					runSingleAgent(
						ctx.cwd,
						agents,
						params.agent!,
